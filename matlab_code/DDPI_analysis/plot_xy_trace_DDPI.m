% pathtodata = 'C:\Users\Ruccilab\Box\Vis_Dynamics\Data\DDPI\old';
...
%     trials.present(i) = ppt{i,1}.present;
        ...
        trials(i).photodiode = data.photocell{i};
        ...
        trials(i).notrack = vector2events(data.triggers{i}.notrack);
        trials(i).blink = vector2events(data.triggers{i}.blink);
...
for i=1:length(ppt)
    ...
    inp = input(prompt);
%     inp = i;
%     Quit = ppt{inp}.quit* (1000/sRate);
%     CueCtr = ppt{inp}.cueCtr* (1000/sRate);
%     StimOFF = ppt{inp}.stimOff* (1000/sRate);
%     SaccON = ppt{inp}.saccOn* (1000/sRate);
%     SaccOFF = ppt{inp}.saccOff* (1000/sRate);
    Quit = ppt{inp}.quit;
...
%     displayTrialSequence_nk_2(ppt,i);
    displayTrialSequence_nk_DDPI(ppt,inp,sRate);
%     legend('off')
    set(gca,'box','off','tickdir','out')
    set(gcf, 'Units', 'Normalized', 'OuterPosition', [0, 0.04, 1, 0.96]);

    subplot(2,1,2);
    tm = ((1:length(ppt{inp}.velocity)) .* (1/sRate) .* 1000);
    plot(tm,ppt{inp}.velocity);
